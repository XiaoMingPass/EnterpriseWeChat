...
public class DateUtil {
    ...
    private static String formatDateByFormat(Date date) {
        String result = "";
        if (date != null) {
            try {
    ...
    public static List<String> getDaysByDate(java.util.Date date) {
        ...
        GregorianCalendar gcLast = (GregorianCalendar) Calendar.getInstance();
        gcLast.setTime(date);
        int daies = DateUtil.getDay(date);
        String monthStr = DateUtil.format(date, "yyyy-MM");
        for (int i = 1; i <= daies; i++) {
            String day;
            if (i < 10) {
                day = monthStr + "-0" + i;
            } else {
    ...
    /**
     * 获取一月 month为1就是上一个月 为-1就是下一个月(Bruce.liu)
     *
     * @param theDate
     * @param month
     * @return
    ...
    public static void main(String[] args) throws Exception {
        System.err.println(new SimpleDateFormat("yyyy-MM-dd mm:HH:ss").format(addMonth(new Date())));
